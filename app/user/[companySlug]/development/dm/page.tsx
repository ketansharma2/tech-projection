...
import UserTaskTable from '@/components/user-task-table'
...
export default function UserDMPage() {
  ...
  const filteredTasks = useMemo(() => {
    return tasksSource.filter((task) => {
      ...
      const matchesMonth = !selectedMonth || task.monthKey === selectedMonth

      return matchesSearch && matchesStatus && matchesActive && matchesMonth
    })
  }, [searchQuery, selectedStatus, activeOnly, selectedMonth, tasksSource])
  ...
  const handleSectionNavigate = (sectionId: string) => {
    // Badge click: accordion behavior - only one section open
    setExpandedSectionIds(new Set([sectionId]))
  ...
  return (
    <main className="pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <TitleBlock title="Digital Marketing" />
          ...
          <div className="space-y-4" style={{ padding: '16px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '16px' }}>
              <div style={{ padding: '8px', borderRadius: '6px', backgroundColor: '#dbeafe', border: '1px solid #93c5fd', textAlign: 'center' }}>
                ...
                <p style={{ fontSize: '18px', fontWeight: 800, color: '#1e3a8a' }}>{dmStats.total}</p>
              </div>
              <div style={{ padding: '8px', borderRadius: '6px', backgroundColor: '#d1fae5', border: '1px solid #6ee7b7', textAlign: 'center' }}>
                ...
                <p style={{ fontSize: '18px', fontWeight: 800, color: '#064e3b' }}>{dmStats.active}</p>
              </div>
              <div style={{ padding: '8px', borderRadius: '6px', backgroundColor: '#ffedd5', border: '1px solid #fdba74', textAlign: 'center' }}>
                ...
                <p style={{ fontSize: '18px', fontWeight: 800, color: '#7c2d12' }}>{dmStats.onHold}</p>
              </div>
                      ...
                      <table className="w-full text-sm" style={{ display: 'table', width: '100%', borderCollapse: 'collapse', fontSize: '10px' }}>
                        <thead>
                          <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
                            ...
                            <th style={{ padding: '6px 8px', textAlign: 'left', fontWeight: 600, fontSize: '9px', color: '#374151', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>Deadline Date</th>
                            ...
                            <th style={{ padding: '6px 8px', textAlign: 'center', fontWeight: 600, fontSize: '9px', color: '#374151', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>Goal/Target</th>
        ...
        {/* Section Navigation */}
        <div className="flex flex-wrap gap-2 mt-6 mb-8 no-print">
          {sections.map((section) => (
            <button
              key={section.id}
        ...
        {/* Sections - Hidden in print, shown in print-only container above */}
        <div className="space-y-4 no-print">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-gray-500 dark:text-gray-400">
              ...
              <p className="text-sm">You don't have any tasks in this section yet.</p>
            </div>
          ) : (
            sections.map((section) => (
              <div key={section.id} id={section.id}>
                <SectionAccordion
                  ...
                  onToggle={(isOpen) => {
                    // Manual click: independent toggle - can open/close multiple
